...
def _isoformat_dict_factory(items: list[tuple]) -> dict:
    """Serialize dataclass fields, converting datetime values to ISO strings."""
    return {k: v.isoformat() if hasattr(v, "isoformat") else v for k, v in items}
...
def _build_ground_truth(gt_dict: dict) -> GroundTruth:
    """Construct a GroundTruth dataclass from a raw ground-truth dict."""
    return GroundTruth(
...
def _score_baseline_file(
    ...
    gt = _build_ground_truth(gt_dict)
    messages = [
...
def _score_prompt_file(
    ...
    """Score one file for a specialised-prompt condition.

    Returns a FileScore directly using normalized bug_type matching.
    Returns None if the file has no GT entry or cannot be read.
    """
    gt_dict = gt_map.get(file_path)
    if gt_dict is None:
        return None
...
def _evaluate_prompt(
    ...
    _build_condition_result's 30/20 split assumption).
    """
    training_files = _list_py_files(TRAINING_BUGS_DIR)
    held_out_files = _list_py_files(HELD_OUT_DIR)
    all_files = training_files + held_out_files

    review_system = f"{prompt}\n\n{REVIEW_JSON_SCHEMA}"
    ...
    results: list[FileScore] = parallel_score_files(score_fn, all_files)
...
def _save_comparison_table(result: ExperimentResult, path: Path) -> None:
    """Write a plain-text Rich comparison table to disk."""
    capture = Console(file=open(path, "w", encoding="utf-8"), width=120)
    print_comparison_table.__globals__["CONSOLE"] = capture
    print_comparison_table(result)
    print_comparison_table.__globals__["CONSOLE"] = CONSOLE
...
def main() -> None:
    ...
    if args.skip_stem:
        ...
        task_theory = dataclasses.asdict(theory_obj, dict_factory=_isoformat_dict_factory)
        stem_config = dataclasses.asdict(config_obj, dict_factory=_isoformat_dict_factory)
    ...
    if args.eval_only:
        ...
        if rl_prompt_path.is_file():
            rl_content = rl_prompt_path.read_text(encoding="utf-8")
            rl_final_prompt = rl_content if rl_content.strip() else baseline_prompt
        ...
        baseline_file_scores = _evaluate_prompt(baseline_prompt, "baseline", review_client, gt_map)
        if condition in ("sft", "all"):
            _panel("Evaluating SFT final prompt...", "cyan")
            sft_file_scores = _evaluate_prompt(sft_final_prompt, "sft", review_client, gt_map)
        if condition in ("rl", "all"):
            _panel("Evaluating RL final prompt...", "bright_green")
            rl_file_scores = _evaluate_prompt(rl_final_prompt, "rl", review_client, gt_map)
    else:
    ...
    print_comparison_table(result)
    _save_comparison_table(result, RESULTS_DIR / "comparison_table.txt")
